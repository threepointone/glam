    ...
    visitor: {
      Program: {
        enter (path, state) {
          ...
          state.inject = function () {
            if (!state.injected) {
              ...
              if (!state.opts.inline && path.hub.file.opts.filename !== 'unknown' && state.opts.import) {
                const basename = require('path').basename
                const cssFilename = './' + basename(path.hub.file.opts.filename) + '.css'
                const impNode = state.opts.sync
                ? t.importDeclaration([], t.stringLiteral(cssFilename))
                : t.expressionStatement(t.callExpression(t.import(), [t.stringLiteral(cssFilename)]))
        ...
        exit (path, state) {
          ...
          const exists = fs.existsSync(file + '.css')
          if (
            !state.opts.inline &&
            state.injected &&
            (exists
